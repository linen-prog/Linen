...
Return ONLY the somatic invitation text. No title, no label, no explanation. Just the 2-4 sentence invitation.`;
        ...
        // Step 6: Return
        app.logger.info({ userId, fallbackUsed: false }, '[Somatic] Fallback used: false — prompt generated by AI');
        return reply.send({
        ...
        app.logger.error({ userId, fallbackUsed: true, errMessage: error instanceof Error ? error.message : 'Unknown error' }, '[Somatic] Fallback used: true — AI generation failed');
