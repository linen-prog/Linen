// Warm Linen Color Palette
const tintColorLight = "#6B7E5E"; // Logo sage (darker)
const tintColorDark = "#ecfdf5"; // Emerald 50

export const zincColors = {
  50: "#fafaf9",  // Stone 50
  100: "#f5f5f4", // Stone 100
  200: "#e7e5e4", // Stone 200
  300: "#d4d4d8",
  400: "#a1a1aa",
  500: "#78716c", // Stone 500
  600: "#57534e", // Stone 600
  700: "#44403c", // Stone 700
  800: "#292524", // Stone 800
  900: "#1c1917", // Stone 900
  950: "#09090b",
};

export const appleBlue = "#007AFF";
export const appleRed = "#FF3B30";

export const borderColor = "#e7e5e4"; // Stone 200
export const appleGreen = "#6B7E5E"; // Logo sage (darker)
export const Colors = {
  light: {
    text: "#1c1917",        // Stone 900
    background: "#fafaf9",  // Stone 50
    tint: tintColorLight,   // Emerald 700
    icon: "#57534e",        // Stone 600
    tabIconDefault: "#57534e",
    tabIconSelected: tintColorLight,
  },
  dark: {
    text: "#fafaf9",        // Stone 50
    background: "#1c1917",  // Stone 900
    tint: tintColorDark,    // Emerald 50
    icon: "#78716c",        // Stone 500
    tabIconDefault: "#78716c",
    tabIconSelected: tintColorDark,
  },
};

export const backgroundColors = [
  "#fef2f2",
  "#fee2e2",
  "#fecaca",
  "#fca5a5",
  "#f87171",
  "#ef4444",
  "#dc2626",
  "#b91c1c",
  "#991b1b",
  "#7f1d1d",

  "#fff7ed",
  "#ffedd5",
  "#fed7aa",
  "#fdba74",
  "#fb923c",
  "#f97316",
  "#ea580c",
  "#c2410c",
  "#9a3412",
  "#7c2d12",

  "#fffbeb",
  "#fef3c7",
  "#fde68a",
  "#fcd34d",
  "#fbbf24",
  "#f59e0b",
  "#d97706",
  "#b45309",
  "#92400e",
  "#78350f",

  "#fefce8",
  "#fef9c3",
  "#fef08a",
  "#fde047",
  "#facc15",
  "#eab308",
  "#ca8a04",
  "#a16207",
  "#854d0e",
  "#713f12",

  "#f7fee7",
  "#ecfccb",
  "#d9f99d",
  "#bef264",
  "#a3e635",
  "#84cc16",
  "#65a30d",
  "#4d7c0f",
  "#3f6212",
  "#365314",

  "#f0fdf4",
  "#dcfce7",
  "#bbf7d0",
  "#86efac",
  "#4ade80",
  "#22c55e",
  "#16a34a",
  "#15803d",
  "#166534",
  "#14532d",

  "#ecfdf5",
  "#d1fae5",
  "#a7f3d0",
  "#6ee7b7",
  "#34d399",
  "#10b981",
  "#059669",
  "#047857",
  "#065f46",
  "#064e3b",

  "#f0fdfa",
  "#ccfbf1",
  "#99f6e4",
  "#5eead4",
  "#2dd4bf",
  "#14b8a6",
  "#0d9488",
  "#0f766e",
  "#115e59",
  "#134e4a",

  "#f0f9ff",
  "#e0f2fe",
  "#bae6fd",
  "#7dd3fc",
  "#38bdf8",
  "#0ea5e9",
  "#0284c7",
  "#0369a1",
  "#075985",
  "#0c4a6e",

  "#eff6ff",
  "#dbeafe",
  "#bfdbfe",
  "#93c5fd",
  "#60a5fa",
  "#3b82f6",
  "#2563eb",
  "#1d4ed8",
  "#1e40af",
  "#1e3a8a",

  "#eef2ff",
  "#e0e7ff",
  "#c7d2fe",
  "#a5b4fc",
  "#818cf8",
  "#6366f1",
  "#4f46e5",
  "#4338ca",
  "#3730a3",
  "#312e81",

  "#f5f3ff",
  "#ede9fe",
  "#ddd6fe",
  "#c4b5fd",
  "#a78bfa",
  "#8b5cf6",
  "#7c3aed",
  "#6d28d9",
  "#5b21b6",
  "#4c1d95",

  "#faf5ff",
  "#f3e8ff",
  "#e9d5ff",
  "#d8b4fe",
  "#c084fc",
  "#a855f7",
  "#9333ea",
  "#7e22ce",
  "#6b21a8",
  "#581c87",

  "#fdf4ff",
  "#fae8ff",
  "#f5d0fe",
  "#f0abfc",
  "#e879f9",
  "#d946ef",
  "#c026d3",
  "#a21caf",
  "#86198f",
  "#701a75",

  "#fdf2f8",
  "#fce7f3",
  "#fbcfe8",
  "#f9a8d4",
  "#f472b6",
  "#ec4899",
  "#db2777",
  "#be185d",
  "#9d174d",
  "#831843",

  "#fff1f2",
  "#ffe4e6",
  "#fecdd3",
  "#fda4af",
  "#fb7185",
  "#f43f5e",
  "#e11d48",
  "#be123c",
  "#9f1239",
  "#881337",
];

// Explicit, theme-independent semantic colors for the Linen aesthetic.
// Use these instead of relying on system dark mode, so text is always readable
// regardless of the device's system appearance setting.
export const LinenColors = {
  background: '#F4EDDF',        // warm cream
  cardBackground: '#FBF5E8',    // ivory card
  cardBackgroundAlt: '#FFFFFF',
  primaryText: '#1c1917',       // soft charcoal (Stone 900)
  secondaryText: '#57534e',     // muted olive-charcoal (Stone 600)
  mutedText: '#78716c',         // muted olive (Stone 500)
  buttonText: '#FFFFFF',
  buttonTextOnLight: '#1c1917',
  placeholder: '#78716c',       // Stone 500
  border: '#e7e5e4',            // Stone 200
  divider: '#e7e5e4',
  accent: '#6B7E5E',            // Logo sage (darker)
  accentDeep: '#5A6F4F',        // Deep sage
  destructive: '#dc2626',       // Red 600
  link: '#6B7E5E',              // Logo sage (darker)
} as const;

export const emojies = [
  // Fruits
  "🍏",
  "🍎",
  "🍐",
  "🍊",
  "🍋",
  "🍌",
  "🍉",
  "🍇",
  "🍓",
  "🫐",
  "🍈",
  "🍒",
  "🍑",
  "🥭",
  "🍍",
  "🥥",
  "🥝",

  // Vegetables
  "🍅",
  "🍆",
  "🥑",
  "🥦",
  "🥬",
  "🥒",
  "🌶",
  "🫑",
  "🌽",
  "🥕",
  "🥔",
  "🧄",
  "🧅",
  "🍄",

  // Breads & Bakery
  "🍞",
  "🥖",
  "🥨",
  "🥐",
  "🥯",

  // Dairy & Eggs
  "🧀",
  "🥚",
  "🍳",
  "🥞",
  "🧇",

  // Meats
  "🥓",
  "🥩",
  "🍗",
  "🍖",

  // Fast Foods
  "🌭",
  "🍔",
  "🍟",
  "🍕",

  // Wraps, Sandwiches & Ethnic Foods
  "🥪",
  "🌮",
  "🌯",
  "🫔",
  "🥙",
  "🧆",

  // Pasta, Rice & Asian Foods
  "🍜",
  "🍝",
  "🍣",
  "🍤",
  "🍙",
  "🍚",
  "🍛",
  "🍲",
  "🥘",
  "🥗",

  // Snacks & Misc
  "🍿",
  "🧈",
  "🥫",
  "🍱",
  "🥮",
  "🍠",
  "🍥",
  "🥟",
  "🥠",
  "🥡",

  // Desserts & Sweets
  "🍦",
  "🍧",
  "🍨",
  "🍩",
  "🍪",
  "🧁",
  "🍰",
  "🎂",
  "🍮",
  "🍭",
  "🍬",
  "🍫",
  "🍯",

  // Nuts
  "🥜",
  "🌰",

  // Drinks
  "🥛",
  "🧃",
  "🧉",
  "🥤",
  "🍶",
  "🍵",
  "🍺",
  "🍻",
  "🥂",
  "🍷",
  "🍸",
  "🍹",
  "🥃",
  "🍾",
  "☕️",
  "🫖",

  // Utensils & Condiments
  "🥄",
  "🍴",
  "🍽",
  "🥢",
  "🧂",

  // Shopping & Payment
  "🛒",
  "🛍️",
  "🧺",
  "💳",
  "💸",
  "💵",
  "💰",
  "💲",
  "🧾",
  "🔖",
  "🏪",
  "🏬",
  "🏦",
  "🏧",
  "📦",
  "📮",
  "🏷️",

  // Organizational / Utility
  "✅",
  "📋",
  "📜",
  "✏️",
  "📝",
  "🔍",
  "📆",
  "⏰",
  "📱",
  "💻",
  "🌐",
  "🔗",
  "🔒",
  "🔑",
  "🗃️",
  "🗂️",
  "🔄",
  "💡",
  "⭐️",
  "📌",
  "📍",
  "📊",
  "💯",
  "🎉",
  "🎊",
  "🎁",
  "🏆",
  "⚖️",
  "🏠",

  // Transportation & Movement (for shopping trips)
  "🚗",
  "🏃‍♂️",
  "🏃‍♀️",
  "🚶‍♂️",
  "🚶‍♀️",

  // Clothing (Items to buy)
  "👕",
  "👖",
  "👗",
  "👔",
  "🩳",
  "👠",
  "👟",
  "🧥",
  "🧤",
  "🧣",
  "🧦",
  "🎒",
  "👜",
  "👛",
  "👓",
  "🕶️",
  "👒",

  // Household Items (Things you might add to a shopping list)
  "🪣",
  "🪑",
  "🛋️",
  "🚪",
  "🪟",
  "🏺",
  "🖼️",
  "📺",
  "📻",
  "🔌",
  "🧴",
  "🪥",
  "🧹",
  "🧽",
  "🗑️",
  "🪒",
  "💊",
  "💉",
  "🩹",
  "❤️",
  "💔",
  "💘",
  "💙",
  "💚",
  "💛",
  "💜",
];
