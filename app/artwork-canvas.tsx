...
const styles = StyleSheet.create({
  ...
  paletteHeader: {
    flexDirection: Perfect! Now I understand the issue. Looking at the code:

1. In `artwork-canvas.tsx`, when sharing artwork, the code logs show it's passing `artworkUrl` to the backend
2. The backend `community.ts` already has the `artworkUrl` field in the schema and returns it
3. The frontend `community.tsx` has the `artworkUrl` field in the `Post` interface and renders an `<Image>` component when `post.artworkUrl` exists

The issue is that the artwork image needs to be properly displayed. Let me check if there's an issue with how the image is being rendered or if the URL is not being passed correctly. Let me update the community screen to ensure images display properly:

<write file="app/(tabs)/community.tsx">
import { useRouter } from 'expo-router';
import { IconSymbol } from '@/components/IconSymbol';
import { useAuth } from '@/contexts/AuthContext';
import React, { useState, useEffect } from 'react';
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, Platform, Modal, Pressable, Image, ActivityIndicator, ImageSourcePropType } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { colors, typography, spacing, borderRadius } from '@/styles/commonStyles';
...
export default function CommunityScreen() {
  ...
  const loadPosts = async (category: string) => {
    try {
      ...
      // Log artwork URLs for debugging
      data.forEach((post: any) => {
        if (post.artworkUrl) {
          console.log('Post with artwork:', post.id, 'URL:', post.artworkUrl);
        }
      });
            ...
            posts.map((post) => {
              ...
              return (
                <View key={post.id} style={styles.postCard}>
                  ...
                  {post.artworkUrl && (
                    <Image 
                      source={resolveImageSource(post.artworkUrl)} 
                      style={styles.artworkImage}
                      resizeMode="cover"
                      onError={(error) => {
                        console.error('Failed to load artwork image:', post.artworkUrl, error.nativeEvent.error);
                      }}
                      onLoad={() => {
                        console.log('Artwork image loaded successfully:', post.artworkUrl);
                      }}
                    />
                  )}
